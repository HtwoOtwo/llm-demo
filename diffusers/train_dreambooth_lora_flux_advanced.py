...
def load_text_encoders(class_one, class_two, weight_dtype):
...
def main(args):
    ...
    # NOTE save memory by directly set model to weight_dtype other than move to
    # import accelerate
    # accelerate.cpu_offload(transformer)
    vae.to(accelerator.device, dtype=weight_dtype)
    quantize(transformer, weights=qfloat8) # NOTE Quantizes on CPU and then moves to GPU
    freeze(transformer)
    transformer.to(accelerator.device)
    quantize(text_encoder_one, weights=qfloat8) # NOTE Quantizes on CPU and then moves to GPU
    freeze(text_encoder_one)
    text_encoder_one.to(accelerator.device)
    quantize(text_encoder_two, weights=qfloat8) # NOTE Quantizes on CPU and then moves to GPU
    ...
    if not freeze_text_encoder:
        ...
        # NOTE you only have one 4090, so DDP is meaningless, save VRAM, accelerator.prepare seems not a inplace operation
        # transformer, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        #     transformer, optimizer, train_dataloader, lr_scheduler
        # )
        train_dataloader = accelerator.prepare(train_dataloader) # move to GPU device
