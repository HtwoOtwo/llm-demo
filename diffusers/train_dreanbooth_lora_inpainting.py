...
def prepare_mask_and_masked_image(image, mask, size):
    ...
    mask = cv2.resize(mask, size, interpolation=cv2.INTER_LINEAR) # resize
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_mask_map = {} # instance mask path: origin image path
    ...
    def __getitem__(self, index):
        ...
        # origin_image = self.image_transforms_resize_and_crop(origin_image)

        # example["PIL_images"] = origin_image
...
def main():
    ...
    unet_parameter_with_lr = {"params": unet_lora_parameters, "lr": args.learning_rate}
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                mask = torch.nn.functional.interpolate(
                    mask, size=(masked_latents.shape[2], masked_latents.shape[3])
                )
